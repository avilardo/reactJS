var handleClick = function(repo) {
  	console.log('You clicked: ' + repo.full_name);
	ReactDOM.unmountComponentAtNode(document.getElementById('commits'));
	ReactDOM.render(
  		<CommitsView commits={$.getJSON('https://api.github.com/repos/' + repo.full_name + '/commits')} projeto={repo.full_name} />, document.getElementById('commits')
	);
...
var RepoItemWrapper = new React.createClass({
	render : function(){
		return(
			<li><a onClick={handleClick.bind(this, this.props.repo)} >{this.props.repo.full_name}</a>{" ( "+ this.props.repo.stargazers_count + " stars / " + this.props.repo.forks_count + " forks)"}</li>
		) 
	}
});
